...
#define N 2        // numar chei
#define NN (N * 2) // numar fii
...
Pagina *insereaza(Pagina *pag, int cheie, Nod *nod)
{
    Nod v;
    int i, stg, dr, mij;
    Pagina *q, *r;
    ...
    if (pag == NULL) // daca nu exista radacina, atunci creăm una cu cheia data ca parametru
    {
        pag = (Pagina *)calloc(sizeof(Pagina), 1);  // avem de lucru doar cu o singura pagina la prima inserare(de aici 1)
        pag->e[++pag->m] = v;

        return pag;
    }

    // se cauta binar pozitia cheii in pagina curenta
    stg = 1;
    dr = pag->m;
    while (stg <= dr)
    {
        mij = (stg + dr) / 2;
        if (pag->e[mij].cheie == cheie) // gasit
        {
            pag->e[mij].contor++;
            return pag;
        }
        if (cheie < pag->e[mij].cheie)
        {
            dr = mij - 1;
        }
        else
    ...
    {
        if (dr == 0) // s-a ajuns la cel mai din stanga element => prima legatura
        {
            q = pag->p0;
        }
        else
