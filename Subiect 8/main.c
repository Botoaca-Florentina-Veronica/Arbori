        ...
        */
        ...
        if (*h == TRUE) // ramura stanga a crescut in inaltime
        {
            ...
            case 1:         // subarborele era dezechilibrat in dreapta
                ...
                *h = FALSE; // deci h rămâne fals, arborele nu crește în înălțime
                break;
            case 0:          // subarborele era in echilibru, iar prin inserarea nodului în subarborele stâng, provocăm dezechilibru în stânga
                p->ech = -1; // dezechilibrat cu un nivel in stanga
                break;
            case -1:               // subarborele era dezechilibrat in stanga cu un nivel,
                                   // acum e cu doua nivele => necesită reechilibrare
                p1 = p->stg;       // aici alegem p1 ca fiind subarborele ce conține cele 3 noduri pe care le vom roti
    ...
    else if (x > p->cheie)
    {
        ...
        if (*h) // ramura dreapta a crescut in inaltime
            ...
            case +1:               // subarborele era dezechilibrat in dreapta cu un nivel,
                ...
                p1 = p->dr;        // la fel ca mai sus, în p1 stocăm acum subarborele drept din arborele nostru, care constă în 3 noduri ce necesită reechilibrare
                ...
                p->ech = 0;
                *h = FALSE; // înălțimea nu se schimbă
            } // switch
    }
    return p;
}
