/*
  Pentru pacientii unui spital, se cunosc ID-ul si anul nasterii. Considerand ca pentru stocarea datelor despre pacienti se foloseste o structura
de tip arbore B, sa se scrie o functie pentru stergerea celui mai tanar pacient.
*/
...
typedef struct Nod {
    TipCheie cheie;
    int an_nastere; // Adăugăm un câmp pentru anul nașterii
    int contor;
    struct Pagina *p;
} Nod;
...
Pagina* insereaza(Pagina *pag, TipCheie x, int an_nastere, Nod *nod)
{
	int i, s, d, mij;
	Pagina *q, *r;
	...
	if (!nod)
	{
		v.cheie = x;
		v.contor = 1;
        v.an_nastere = an_nastere; // Salvăm anul nașterii
		v.p = NULL;
	}
	else
		v = *nod;
	...
	else
	{
		if (d == 0) //s-a ajuns la cel mai din stanga element => prima legatura
			...
			q = pag->e[d].p;
		r = insereaza(q, x, an_nastere, NULL);
		if (r != q) //daca pagina in care s-a inserat s-a scindat <=> arborele crescut cu un nivel
		{
			...
			pag = insereaza(pag, r->e[1].cheie, r->e[1].an_nastere, &r->e[1]);
...
void afisare(Pagina *arbore, int nivel)
{
    ...
    for (i = 1; i <= arbore->m; i++)
        printf("ID=%d, An nastere=%d ", arbore->e[i].cheie, arbore->e[i].an_nastere);
    printf("\n");
}
...
TipCheie gasesteCeaMaiMareCheie(Pagina *pag)
{
    while (pag->p0 != NULL)
    {
        pag = pag->e[pag->m].p;
...
void stergeCelMaiTanarPacient(Pagina **arbore)
{
    TipCheie ceaMaiMicaCheie = gasesteCeaMaiMareCheie(*arbore);
    *arbore = suprima(*arbore, ceaMaiMicaCheie, NULL);
}

int main(void)
{
    // Arborele B inițial este gol
    Pagina *arbore = NULL;

    // Introduceți aici logica pentru inițializarea arborelui și adăugarea de pacienți
    arbore = insereaza(arbore, 1001, 1990, NULL); // Exemplu de inserare a unui pacient cu ID-ul 1001 și anul nașterii 1990
    arbore = insereaza(arbore, 1002, 1985, NULL); // Exemplu de inserare a unui pacient cu ID-ul 1002 și anul nașterii 1985
    arbore = insereaza(arbore, 1003, 2000, NULL); // Exemplu de inserare a unui pacient cu ID-ul 1003 și anul nașterii 2000

    printf("Arbore inainte de stergere:\n");
    afisare(arbore, 0);

    // Stergerea celui mai tânăr pacient
    stergeCelMaiTanarPacient(&arbore);

    // Afișarea arborelui actualizat
    printf("Arbore dupa stergerea celui mai tanar pacient:\n");
    afisare(arbore, 0);
